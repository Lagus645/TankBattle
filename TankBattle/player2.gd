...
func _physics_process(delta):
	if is_dead == true:
		return
	if Input.is_action_pressed("down2") and Input.is_action_pressed("left2"):
		velocity.x = -SPEED / sqrt(2)
		velocity.y = SPEED / sqrt(2)
		$".".rotation = deg2rad(-135)
		$AnimatedSprite.play("go")
		$AudioStreamPlayer2.play()
	elif Input.is_action_pressed("down2") and Input.is_action_pressed("right2"):
		velocity.x = SPEED / sqrt(2)
		velocity.y = SPEED / sqrt(2)
		$".".rotation = deg2rad(135)
		$AnimatedSprite.play("go")
		$AudioStreamPlayer2.play()
	elif Input.is_action_pressed("up2") and Input.is_action_pressed("right2"):
		velocity.x = SPEED / sqrt(2)
		velocity.y = -SPEED / sqrt(2)
		$".".rotation = deg2rad(45)
		$AnimatedSprite.play("go")
		$AudioStreamPlayer2.play()
	elif Input.is_action_pressed("up2") and Input.is_action_pressed("left2"):
		velocity.x = -SPEED / sqrt(2)
		velocity.y = -SPEED / sqrt(2)
		$".".rotation = deg2rad(-45)
		$AnimatedSprite.play("go")
	...
	if Input.is_action_just_pressed("accept2") && is_firing == false:
		is_firing = true
